...
class PokemonAINetwork2(object):
	...
	def _network_model(self):
		...
		with tf.variable_scope('Split'):
			...
			pokemon_mix = [embedding_input[0] for embedding_input in embedding_inputs]
			item_mix = [embedding_input[5] for embedding_input in embedding_inputs]
			status_mix = [embedding_input[6] for embedding_input in embedding_inputs]
		...
		with tf.variable_scope('FCCOwnPoke'):
			for layer in range(self.config.hidden_layers_poke_vecs):
				...
				W = tf.get_variable('W' + str(i), shape=(inp_size,self.config.hidden_dim_poke_vecs))
				b = tf.get_variable('b' + str(i), shape=(self.config.hidden_dim_poke_vecs,))
		...
		with tf.variable_scope('FCCOppPoke:'):
			for layer in range(self.config.hidden_layers_poke_vecs):
				...
				W = tf.get_variable('W' + str(i), shape=(inp_size,self.config.hidden_dim_poke_vecs))
				b = tf.get_variable('b' + str(i), shape=(self.config.hidden_dim_poke_vecs,))
				opp_team_poke = [tf.nn.relu(tf.matmul(poke,W) + b) for poke in own_team_poke]
		# opp_team_poke a list of size 6 containing tensors of shape (None, self.confid.hidden_dim_poke_vecs)
		curr_own_poke = own_team_poke[0]
		curr_opp_poke = opp_team_poke[0]
		with tf.variable_scope('OwnTeam'):
			team_vecs = tf.stack(own_team_poke[1:], axis=1)
			team_max = tf.reduce_max(team_vecs, axis=1)
			team_min = tf.reduce_min(team_vecs, axis=1)
			team_mean = tf.reduce_mean(team_vecs, axis=1)
			own_team = tf.concat([team_min, team_max, team_mean], 1)
		with tf.variable_scope('OppTeam'):
			team_vecs = tf.stack(opp_team_poke[1:], axis=1)
			team_max = tf.reduce_max(team_vecs, axis=1)
			team_min = tf.reduce_min(team_vecs, axis=1)
			team_mean = tf.reduce_mean(team_vecs, axis=1)
			opp_team = tf.concat([team_min, team_max, team_mean], 1)
