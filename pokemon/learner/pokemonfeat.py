...
class PokemonAINetwork(object):
	...
	def _add_loss(self):
		self.possible_actions = tf.squeeze(tf.slice(self.possible_actions_placeholder, [0,0,1], [-1,-1,-1]), axis=2)
		...
		self.indexes_nd = tf.transpose(tf.stack((row_selector, col_selector), -1))
